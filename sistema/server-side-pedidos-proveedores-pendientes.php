...
$column = array("pedidos.id_pedido", "clientes.dni", "pedidos.id_usuario","pedidos.fecha_registro", 
"pedidos.fecha_entrega", "pedidos.dir_entrega", "pedidos.observacion", "pedidos.precio_total",
"pedidos.estado","estado.estados");
$query = "
 SELECT * FROM pedidos 
 INNER JOIN clientes 
 ON clientes.dni = pedidos.id_cliente 
 INNER JOIN estado 
 ON pedidos.estado = estado.idEstado 
";
$query .= " WHERE ";
if(isset($_POST["ped"]))
{
 $query .= "pedidos.id_cliente  = '".$_POST["ped"]."' AND ";
}
if(isset($_POST["search"]["value"]))
{
 $query .= '(pedidos.id_pedido LIKE "%'.$_POST["search"]["value"].'%" ';
 $query .= 'OR clientes.dni LIKE "%'.$_POST["search"]["value"].'%" ';
...
while($row = mysqli_fetch_array($result))
{
 ...
 $sub_array[] = $row["dir_entrega"];
 $sub_array[] = $row["observacion"];
 $sub_array[] = $row["estados"];
 $sub_array[] = $row["precio_total"];
 $sub_array[] = " <form style='border:none; padding: 0px;'>
 <input type='submit'  class='btnAdd' value='Pedido' name='btn_add' formaction='datos_cliente_compra_proveedores.php'>
 <input type='submit'  class='btnAdd' value='Detalles' name='btn_add' formaction='detalles_pedidos.php'>
