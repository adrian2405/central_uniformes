...
$column = array(
    "pedidos.id_pedido", "usuario.nombre", "pedidos.id_usuario", "pedidos.fecha_registro",
    "pedidos.fecha_entrega", "pedidos.dir_entrega", "pedidos.observacion", "pedidos.precio_total",
    "pedidos.estado", "estado.estados"
);
$query = "
 SELECT * FROM pedidos 
 INNER JOIN usuario 
 ON usuario.idusuario = pedidos.id_usuario 
 INNER JOIN estado 
 ON estado.idEstado = pedidos.estado
";
$query .= " WHERE ";
if (isset($_POST["pend"])) {
    $query .= "estado.estados = '" . $_POST["pend"] . "' AND ";
}
if (isset($_POST["search"]["value"])) {
    $query .= '(pedidos.id_pedido LIKE "%' . $_POST["search"]["value"] . '%" ';
    $query .= 'OR pedidos.id_usuario LIKE "%' . $_POST["search"]["value"] . '%" ';
...
while ($row = mysqli_fetch_array($result)) {
 ...
 <input type='submit'  class='btnAdd' style='background: #65ADEC; color: white; margin-bottom: 1.5px' value='Pedido' name='btn_add' formaction='datos_cliente_compra_proveedores.php'>
 <input type='submit'  class='btnAdd' style='background: #65ADEC; color: white;' value='Detalles' name='btn_add' formaction='detalles_pedidos.php'>
